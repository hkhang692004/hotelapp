...
class PaymentService:
    ...
    def _render_invoice_pdf(invoice):
        ...
        summary_rows = [
            [Paragraph('Chi phí phòng', styles['InvoiceText']), Paragraph(f'{room_total:,.0f} đ', styles['InvoiceText'])],
        ]
    ...
    def _send_invoice_email(invoice_id):
        ...
        try:
            ...
            logger.exception('PDF font registration failed for invoice %s', invoice_id)
        ...
        try:
            ...
            logger.exception('PDF generation failed for invoice %s, will send email without attachment', invoice_id)
            pdf_bytes = None
        ...
        text_body = (
            ...
            f'Chi phí phòng: {room_total:,.0f} đ\n'
        )
        if service_total > Decimal('0'):
        ...
        <div style="font-family:Arial,sans-serif;color:#222;line-height:1.6">
          ...
          <p style="margin:0 0 4px"><b>Chi phí phòng:</b> {room_total:,.0f} đ</p>
          {service_fee_row}
