...
from app.models import Booking, BookingRoom, BookingStatusHistory
...
class BookingDetailSerializer(serializers.ModelSerializer):
    ...
    customer_guest_profile = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = (
            ...
            'customer_guest_profile',
            'checked_in_at', 'checked_out_at', 'cancelled_at', 'cancel_reason', 'created_at',
        )

    def get_customer_guest_profile(self, obj):
        profile = getattr(obj.customer, 'guest_profile', None)
        if not profile:
            return None
        return {
...
class BookingCheckInSerializer(serializers.Serializer):
    ...
    def validate_national_id(self, value):
        v = (value or '').strip()
        if not v:
            raise serializers.ValidationError('CCCD/Passport là bắt buộc')
        return v

    def validate_address(self, value):
        v = (value or '').strip()
        if not v:
            raise serializers.ValidationError('Địa chỉ là bắt buộc')
        return v
